...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $tanggal_pinjam = !empty($_POST['tanggal_pinjam']) ? $_POST['tanggal_pinjam'] : date('Y-m-d'); // Default ke hari ini
    $tanggal_kembali = !empty($_POST['tanggal_kembali']) ? $_POST['tanggal_kembali'] : null; // Tetap null jika kosong
    ...
    $query = "INSERT INTO peminjaman (id_anggota, id_petugas, tanggal_pinjam, tanggal_kembali, id_buku)
              VALUES ('$id_anggota', '$id_petugas', '$tanggal_pinjam', " . ($tanggal_kembali ? "'$tanggal_kembali'" : "NULL") . ", '$id_buku')";
    $conn->exec($query);
...
<body>
    ...
    <form method="POST">
        ...
        <input type="date" id="tanggal_kembali" name="tanggal_kembali"><br>
